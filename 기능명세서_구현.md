...
> **최종 업데이트:** 2026-09-19 (문서 동기화 - 구현 현황 최신화)
...
#### 2.6 동영상 인터랙션 ⭐ **NEW**
- [x] 좋아요/싫어요 버튼 UI
- [x] YouTube API 좋아요/싫어요 등록
- [x] 현재 사용자 평가 상태 조회
- [x] 실시간 좋아요 수 업데이트 (로컬 카운트 추적)
- [ ] 댓글 보기 버튼
- [ ] 댓글 목록 조회 (인기순/최신순)
- [ ] 댓글 하단 시트 UI
- [ ] 대댓글 표시 및 펼치기/접기
- [ ] 댓글 페이지네이션
- **구현 메서드:** `rateVideo()`, `getVideoRating()`
- **파일:** `lib/services/api/youtube_service.dart`, `lib/views/screens/player_screen.dart`

#### 2.7 하이브리드 플레이어 시스템 ⭐ **NEW**
- [x] YouTube IFrame 플레이어 (기본)
- [x] 대체 플레이어 시스템 (youtube_explode_dart + chewie)
- [x] Error 150 자동 감지 및 전환
- [x] 직접 스트림 URL 추출
- [x] 로딩 상태 UI
- [x] 통합 재생/일시정지/탐색 인터페이스
- [x] 타이머 서비스 통합 (두 플레이어 모두)
- [x] 에러 처리 및 재시도 옵션
- [x] YouTube 앱으로 열기 옵션
- **기술 스택:** `youtube_player_flutter`, `youtube_explode_dart`, `video_player`, `chewie`
- **파일:** `lib/widgets/player/youtube_player_widget.dart`
- **제한사항:** 대체 플레이어는 360p 최대 화질 (muxed stream 특성)
...
#### 4.1 YouTube 플레이어 위젯 ⭐ **업데이트**
- [x] YouTube IFrame Player 구현
- [x] YouTube 기본 컨트롤 사용
- [x] 재생 시간 Slider (탐색 가능)
- [x] ~~커스텀 재생/일시정지 버튼~~ (제거됨 - 2026-03-14)
- [x] ~~10초 앞으로/뒤로 버튼~~ (제거됨 - 2026-03-14)
- **파일:** `lib/widgets/player/youtube_player_widget.dart`
- **변경 이유:** YouTube 기본 컨트롤과 중복 제거, UI 간소화
...
- [x] ~~실시간 타이머 상태 UI 표시~~ → 제거됨 (타이머는 백그라운드 동작, UI 비표시)
- [x] 퀴즈 네비게이션 시 타이머 제어
- [x] 화면 종료 시 타이머 정리
...
- [x] **정답 시 자동 복귀** (1.5초 후 자동으로 영상 재생 화면 복귀, Issue #72)
  - player_screen: 오버레이 모드에서 정답 시 1.5초 후 `_hideQuestionOverlay()` 자동 호출
  - question_screen: 다이얼로그 모드에서 정답 시 1.5초 후 자동 닫기 + `_returnToVideo()` 호출
  - Timer를 state 변수로 관리, dispose 시 cancel 처리
  - 사용자가 자동 복귀 전 버튼 클릭 시 타이머 취소 후 즉시 복귀
...
### 1. AI 퀴즈 고도화
- ✅ OpenAI API 연동 및 동적 문제 생성 (코드 경로 구현 완료)
  - `.env`에 실제 키가 있으면 `_generateQuestionWithAI()` 경로로 동작
  - 플레이스홀더(`YOUR_OPENAI_API_KEY`)면 내장 문제은행으로 자동 폴백
  - API 호출 실패 시에도 문제은행 폴백 (`question_generator_service.dart:95`)
- ⏳ 난이도 자동 조정 알고리즘 (#23, #48~51)

**남은 작업:**
- 정답률 추적 기반 난이도 자동 조정
- 프롬프트 엔지니어링 최적화 및 생성 품질 검증

---

### 2. 사용자 프로필 완성
> 설정 화면(학년/타이머/저장)은 #52~#57로 구현 완료. 아래는 프로필 화면 잔여 항목.
- ⏳ 학습 수준 설정 UI
- ⏳ 선호 과목/취약 과목 관리
- ⏳ 프로필 사진 업로드 기능
...
### 코드 품질
- [x] 단위 테스트 작성 (70개 테스트 케이스, models/services 커버리지)
- [x] GitHub Actions CI 파이프라인 (flutter analyze + flutter test)
- [x] /test 스킬 (analyze/test 결과 보고 및 PR 가능 여부 판단)
- [ ] 통합 테스트 작성
- [ ] 코드 리뷰 프로세스 도입
- [ ] Linting 규칙 강화
...
│   │       ├── profile_screen.dart   # ⏳ 스텁 (16줄)
│   │       ├── settings_screen.dart  # ✅ 완성
│   │       ├── grade_settings_screen.dart # ✅ 완성
│   │       └── timer_settings_screen.dart # ✅ 완성
│   ├── widgets/
│   │   └── player/
...
### 2026-09-19: 상표 리스크 대응 - UI 차별화 및 법적 고지 (#82~#85)
- ✅ **#82** 'Shorts' 용어 제거 → '클립'으로 변경 (탭 라벨, 배지, 오류/빈 상태 메시지)
- ✅ **#83** 하단 네비게이션 재설계: 홈/Shorts/보관함/설정 → 탐색/클립/내 학습/설정
  - 아이콘을 교육 테마로 교체 (explore / bolt / school / tune)
  - 선택 색상을 브랜드 컬러로 변경
- ✅ **#84** AppBar 미구현 버튼(Cast, 알림) 제거, 검색+프로필만 유지
  - 영상 카드에서 원형 아바타 + 3dot 메뉴 구조 제거
  - 카드형 레이아웃(둥근 모서리/그림자) 및 '학습 영상' 배지 적용
- ✅ **#85** 설정 > 앱 정보에 비공식 앱 고지, Powered by YouTube,
  YouTube 약관/Google 개인정보처리방침 링크 추가
- **미포함**: AppBar 타이틀의 'YouTube' 텍스트와 앱 이름은 #81 범위

### 2026-09-19: 플랫폼 전략 확정 - 자체 플레이어 통일 (ADR-001)
- ✅ **결정**: Android/iOS 모두 앱 내 자체 플레이어로만 재생.
  OS 레벨 YouTube 본앱 감시·정지·오버레이 방식은 채택하지 않음
- ✅ 기술 검토: iOS는 시스템 전역 오버레이 API 부재, 포그라운드 앱 감지 불가,
  타 앱 재생 제어 불가 → 3중 봉쇄로 동일 동작 구현 불가능
- ✅ iOS Screen Time API는 "차단"만 가능하고 실드 UI 커스터마이징 불가,
  `ShieldActionDelegate`에서 앱 실행 불가 (iOS 27에서도 미개선)
- ✅ Android 17 Advanced Protection Mode는 자녀보호 앱 포함 비접근성 앱의
  AccessibilityService를 전면 차단 → 접근성 API 의존 설계 회피
- ✅ 기능명세서 1.5절에 ADR-001로 결정·근거·트레이드오프 기록
- **코드 영향 없음**: 현재 구현(`player_screen.dart` 자체 플레이어 + 타이머 인터럽트)이
  이미 본 결정에 부합하며, 별도 마이그레이션 불필요
- **후속 과제**: 이탈 방지 보완책(명세서 1.5.3)은 Phase 2 선택 기능으로 분리

### 2026-03-28: 퀴즈 정답 시 자동 복귀 (Issue #72)
- ✅ player_screen: 정답 시 1.5초 후 오버레이 자동 닫기 + 영상 재생 재개
- ✅ question_screen: 정답 시 1.5초 후 다이얼로그 자동 닫기 + 영상 복귀
- ✅ Timer state 변수 관리 및 dispose 시 정리
- ✅ 수동 버튼 클릭 시 타이머 즉시 취소

### 2025-12-27: 학습 타이머 시스템 개선 및 UX 향상
- ✅ 타이머 간격 15초로 단축 (테스트 용이성)
- ✅ 퀴즈 진입 시 타이머 자동 일시정지
- ✅ 퀴즈 종료 시 타이머 자동 재시작
- ✅ Riverpod 기반 타이머 서비스 통합
- ✅ 실시간 타이머 상태 UI 업데이트
- ✅ 타이머를 영상 재생 시작 시에만 시작하도록 변경
- ✅ 학습 시간 팝업 닫힘 문제 수정 (ref.listen 사용)
- ✅ 퀴즈 완료 후 영상 자동 재생 기능 추가
- ✅ GlobalKey를 사용한 플레이어 제어 구현
